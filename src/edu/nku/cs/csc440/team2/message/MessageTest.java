...
public class MessageTest extends ActivityInstrumentationTestCase2<HelloAndroid> {
	...
	protected void setUp() throws Exception {
		...
		System.out
				.println("* MessageTest: test setUpClass - create timing containers");
		s1 = new Sequence();
		p1 = new Parallel();
		s2 = new Sequence(1.0);
		p2 = new Parallel(1.0);
		...
		System.out
				.println("* MessageTest: test setUpClass - create media elements");
		...
		System.out
				.println("* MessageTest: test setUpClass - add elements to containers");
		s1.addElement(t1);
		p1.addElement(i1);
		s2.addElement(a1);
		p2.addElement(v1);
		System.out
				.println("* MessageTest: test setUpClass - add containers to containers");
		s1.addElement(p1);
		p2.addElement(s2);
		System.out
				.println("* MessageTest: test setUpClass - add elements and containers to implicit sequence");
		m1.addElement(t2);
		m1.addElement(i2);
		m1.addElement(s1);
		m1.addElement(p2);
	...
	public void testAll() throws Exception {
		...
		// getRootLayoutCheck
		System.out
				.println("* MessageTest: test method 3 - getRootLayoutCheck()");
		assertEquals(new SmilDimension(123, 456), m1.getRootLayout()
				.getDimensions());
		assertEquals("pink", m1.getRootLayout().getBackgroundColor());
		assertEquals(new SmilDimension(480, 320), m2.getRootLayout()
				.getDimensions());
		assertEquals("#9ACD32", m2.getRootLayout().getBackgroundColor());
		assertEquals(new SmilDimension(480, 320), m3.getRootLayout()
				.getDimensions());
		assertEquals("transparent", m3.getRootLayout().getBackgroundColor());

		// getDownloadPriority
		System.out
				.println("* MessageTest: test method 4 - getDownloadPriority");
		assertEquals(null, m1.getDownloadPriority());
		assertEquals(null, m2.getDownloadPriority());
		assertEquals(null, m3.getDownloadPriority());

		// getAllElementIdsCheck
		System.out
				.println("* MessageTest: test method 5 - getAllElementIdsCheck()");
		assertEquals("[a1, a2, i1, i2, p1, p2, s1, s2, t1, t2, v1, v2]", m1
				.getAllElementIds().toString());
		...
		// removeElementCheck
		System.out
				.println("* MessageTest: test method 9 - removeElementCheck()");
		...
		assertEquals("[a2, a3, i2, i4, p2, s2, t3, v1, v4]", m1
				.getAllElementIds().toString());
		...
		f = m1.toFile(Environment.getExternalStorageDirectory() + "/temp.smil");
		m4 = new Message(f);
		System.out.println("* \trepeat getIdCheck()");
		assertEquals("good_id", m4.getId());
		System.out.println("* \trepeat getRootLayoutCheck()");
		assertEquals(new SmilDimension(123, 456), m4.getRootLayout()
				.getDimensions());
		...
		System.out.println("* \trepeat getAllElementIdsCheck()");
		assertEquals("[a2, a3, i2, i4, p2, s2, t3, v1, v4]", m4
				.getAllElementIds().toString());
		System.out.println("* \trepeat getBodyCheck()");
